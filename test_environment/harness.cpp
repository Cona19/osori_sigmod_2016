...
const unsigned long MAX_FAILED_QUERIES = 1;
...
int run_harness(const char *init_file_path, const char *workload_file_path, const char *result_file_path, const char *exe_file_path, double *elapsed_sec, unsigned long *cnt_failure)
{
	...
	{
		std::ifstream work_file(workload_file_path);
		if (!work_file)
		{
	...
	else if (pid == 0)
	{
		...
		close(stdout_pipe[0]);
		close(stdout_pipe[1]);
		execlp(exe_file_path, exe_file_path, (char *) NULL);
		perror("execlp");
		exit(EXIT_FAILURE);
	}
	close(stdin_pipe[0]);
	...
	int init_file = open(init_file_path, O_RDONLY);
	if (init_file == -1)
	{
